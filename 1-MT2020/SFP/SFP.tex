...
\begin{document}
	...
	We note that \lecnr{7} $J _{\pm}J _{\mp}$ are of the form $O^\dagger O$, thus the eigenvalues are non-negative, with zero eigenvalues only if $J_+$ or $J_-$ annihilates the state (i.e. $\mel{\psi}{O^\dagger O}{\psi} \geq 0$ with $=0$ only if $O \ket{\psi}=0$). Hence there exists a $m _{\text{max}}$ and $m _{\text{min}}$ such that
	\[
